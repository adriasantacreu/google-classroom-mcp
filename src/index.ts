...
class GoogleClassroomServer {
  ...
  constructor() {
    this.server = new Server(
      { name: 'google-classroom-mcp', version: '2.0.0' },
      { capabilities: { tools: {} } }
    );
    this.setupHandlers();
  }
  ...
  private async handleToolCall(
    ...
    switch (name) {
      ...
      case 'classroom_update_course': {
        ...
        if (args.name)        { body.name = args.name;               maskFields.push('name'); }
        if (args.section)     { body.section = args.section;         maskFields.push('section'); }
        if (args.description) { body.descriptionHeading = args.description; maskFields.push('descriptionHeading'); }
        if (args.room)        { body.room = args.room;               maskFields.push('room'); }
        if (args.state)       { body.courseState = args.state;       maskFields.push('courseState'); }
      ...
      case 'classroom_grade_submission': {
        const gradeBody: any = args.draft
      ...
      case 'classroom_list_topics': {
        const items: classroom_v1.Schema$Topic[] = [];
        let pageToken = args.pageToken;
        do {
      ...
      case 'classroom_list_materials': {
        const items: classroom_v1.Schema$CourseWorkMaterial[] = [];
        let pageToken = args.pageToken;
        do {
